...
void DecodeBBox(
  ...
  // NormalizedBbox &bbox -> prediction bbox
  if (code_type == PriorBoxParameter_CodeType_CORNER) {
    ...
    // prior box format: [x1, y1, x2, y2] -> [Xc, Yc, w, h]
    float prior_width = prior_bbox.xmax() - prior_bbox.xmin();
    CHECK_GT(prior_width, 0);
    float prior_height = prior_bbox.ymax() - prior_bbox.ymin();
    CHECK_GT(prior_height, 0);
    ...
    /* x, y -> Xcenter, Ycenter
	 * tw = log(Pw/Aw), th = log(Ph/Ah), tx = (Px - Ax) / Aw, ty = (Py - Ay) / Ah
     * Pw = e^tw * Aw, Ph = e^th * Ah, Px = tx * Aw + Ax, Py = ty * Ah + Ay
 	 */ 
    if (variance_encoded_in_target) {
      // note: variance is encoded in target, we simply need to restore the offset predictions.
      decode_bbox_center_x = bbox.xmin() * prior_width + prior_center_x;
      decode_bbox_center_y = bbox.ymin() * prior_height + prior_center_y;
      decode_bbox_width = exp(bbox.xmax()) * prior_width;
      decode_bbox_height = exp(bbox.ymax()) * prior_height;
    } else {
      // note: variance is encoded in bbox, we need to scale the offset accordingly.
      // bbox: xmin -> Xc 
	  // bbox: ymin -> Yc
	  // bbox: xmax -> width
	  // bbox: ymax -> height
...
void FindMatches(const vector<LabelBBox>& all_loc_preds,
  ...
  //std::cout << "test encode_variance_in_target value: " << encode_variance_in_target << std::endl;
  const bool ignore_cross_boundary_bbox =
...
void ApplyNMSFast(const Dtype* bboxes, const Dtype* scores, const int num,
  ...
  //std::cout << score_index_vec.size() << "!!!!!!!!!!!!!!!!!" << std::endl;
  //for (int i = 0; i < score_index_vec.size(); i++) {
  //  std::cout << score_index_vec[i].first << " " << score_index_vec[i].second << std::endl;
  //}
